...
impl SttPool {
    pub async fn new(
        ...
        Ok(Self {
            data: Rc::new(RefCell::new(
                SttPoolData::new(initial_size, capacity, lang, prefer_online, ibm_data).await?,
            )),
            lang: lang.clone(),
            prefer_online,
    ...
    async fn take(&mut self) -> Result<SttPoolItem> {
        Ok(SttPoolItem {
            ...
            value: Some(
                SttFactory::load(&self.lang, self.prefer_online, self.ibm_data.clone()).await?,
            ),
...
impl DerefMut for SttPoolItem {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.value
            .as_mut()
            .expect("Tried to access an empty pool item")
    }
}
...
impl Drop for SttPoolItem {
    fn drop(&mut self) {
        if let Some(pool) = self.pool.upgrade() {
            pool.borrow_mut()
                .return_val(self.value.take().expect("Tried to drop an empty pool item"));
        }
    }
}
