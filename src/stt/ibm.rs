...
use crate::stt::{
    DecodeRes, OnlineSttError, Stt, SttBatched, SttConstructionError, SttError, SttInfo,
};
...
impl IbmStt {
    pub async fn new(
        ...
        data: IbmSttData,
    ) -> Result<Self, SttConstructionError> {
        Ok(IbmStt {
            engine: IbmSttEngine::new(data).await,
            model: Self::model_from_lang(lang)?.to_string(),
        })
    }
    ...
    fn lang_neg(lang: &LanguageIdentifier) -> LanguageIdentifier {
        let available_langs = langids!(
            "ar-AR", "ar-MS", "cs-CZ", "de-DE", "en-AU", "en-GB", "en-IN", "en-US", "es-AR",
            "es-CL", "es-CO", "es-ES", "es-MX", "es-PE", "fr-CA", "fr-FR", "hi-IN", "it-IT",
...
impl SttBatched for IbmStt {
    async fn decode(&mut self, audio: &[i16]) -> Result<Option<DecodeRes>, SttError> {
        Ok(self
            .engine
            .decode(
                &AudioRaw::new_raw(audio.to_vec(), DEFAULT_SAMPLES_PER_SECOND),
                &self.model,
            )
            .await?)
    }
    ...
    fn get_info(&self) -> SttInfo {
        SttInfo {
            name: "Ibm's Speech To Text".to_string(),
            is_online: true,
        }
    }
}
...
impl Stt for IbmStt {
    ...
    async fn process(&mut self, audio: &[i16]) -> Result<(), SttError> {
        self.engine
            .live_process(&AudioRaw::new_raw(audio.to_vec(), 16000))
            .await?;
        Ok(())
    }
...
impl WatsonSocket {
    async fn new(model: &str, data: IbmSttData, token: &str) -> Result<Self, OnlineSttError> {
        let url_str = format!(
            "wss://api.{}.speech-to-text.watson.cloud.ibm.com/instances/{}/v1/recognize",
            data.gateway, data.instance
        );
        let (socket, _response) = connect_async(Url::parse_with_params(
            &url_str,
            &[("access_token", token), ("model", model)],
        )?)
        .await?;
    ...
    async fn send_order(&mut self, order: WatsonOrder) -> Result<(), OnlineSttError> {
        ...
        let order = match order {
            WatsonOrder::Start => WatsonOrderInternal {
                action: "start",
                content_type: Some("audio/ogg"),
            },
            WatsonOrder::Stop => WatsonOrderInternal {
                action: "stop",
                content_type: None,
            },
        };
    ...
    async fn send_audio(&mut self, audio: &AudioRaw) -> Result<(), OnlineSttError> {
        ...
        self.socket.send(Message::Binary(as_ogg)).await?;
    ...
    async fn get_answer(&mut self) -> Result<Option<DecodeRes>, OnlineSttError> {
        loop {
            if let Message::Text(response_str) = self
                .socket
                .next()
                .await
                ...
                if let Ok(response) = response_res {
                    let res = {
                        if !response.results.is_empty() {
                            let alternatives =
                                &response.results[response.result_index as usize].alternatives;
...
impl TokenCache {
    async fn new(key: &str) -> Self {
        ...
        if let Err(err) = res.get(key).await {
            warn!(
                "Initial IBM API key couldn't be obtained, continuing regardless: {:?}",
                err
            );
        }
    ...
    async fn get(&mut self, key: &str) -> Result<&str, OnlineSttError> {
        ...
        if must_redo {
            ...
            self.data = Some((
                token,
                Instant::now() + Duration::new((valid_time - (2 * 60)).into(), 0),
            ));
        }
...
impl IbmSttEngine {
    pub async fn new(data: IbmSttData) -> Self {
        ...
        IbmSttEngine {
            curr_socket: None,
            token_cache: TokenCache::new(&data.key).await,
            data: IbmSttData {
                key: data.key,
    ...
    pub async fn decode(
        ...
        audio: &AudioRaw,
        model: &str,
    ) -> Result<Option<DecodeRes>, OnlineSttError> {
        let mut socket = WatsonSocket::new(
            model,
            self.data.clone(),
            self.token_cache.get(&self.data.key).await?,
        )
        .await?;
    ...
    pub async fn live_process(&mut self, audio: &AudioRaw) -> Result<(), OnlineSttError> {
        let socket = self
            .curr_socket
            .as_mut()
            .expect("IbmSttEngine.live_process can't be called before live_proces_begin");
        socket.send_audio(audio).await?;
    ...
    pub async fn live_process_end(&mut self) -> Result<Option<DecodeRes>, OnlineSttError> {
        let socket = self
            .curr_socket
            .as_mut()
            .expect("live_process_end can't be called twice");
