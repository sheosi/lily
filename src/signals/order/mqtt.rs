...
impl MqttInterfaceIn {
    ...
    pub async fn handle_new_sattelite(
        &mut self,
        payload: &Bytes,
        config: &Config,
        ...
        Ok(client
            .lock_it()
            .publish("lily/satellite_welcome", QoS::AtMostOnce, false, output)
            .await?)
    }

    pub async fn handle_nlu_process(
        &mut self,
        payload: &Bytes,
    ...
    pub async fn subscribe(client: &Arc<Mutex<AsyncClient>>) -> Result<()> {
        ...
        client_raw
            .subscribe("lily/new_satellite", QoS::AtMostOnce)
            .await?;
        client_raw
            .subscribe("lily/nlu_process", QoS::AtMostOnce)
            .await?;
        client_raw.subscribe("lily/event", QoS::AtMostOnce).await?;
        client_raw
            .subscribe("lily/disconnected", QoS::ExactlyOnce)
            .await?;
...
impl MqttInterfaceOut {
    ...
    pub async fn handle_out(
        ...
        let voice_prefs: VoiceDescr = VoiceDescr {
            gender: if conf_tts.prefer_male {
                Gender::Male
            } else {
                Gender::Female
            },
        ...
        for lang in curr_langs {
            let tts = TtsFactory::load_with_prefs(
                lang,
                conf_tts.prefer_online,
                conf_tts.ibm.clone(),
                &voice_prefs,
            )?;
    ...
    async fn process_out(
        ...
        let msg_pack = encode::to_vec(&MsgAnswer {
            audio: Some(audio_data.into_encoded()?),
            text: None,
        })?;
        client
            .lock_it()
            .publish(
                &format!("lily/{}/say_msg", uuid_str),
                QoS::AtMostOnce,
                false,
                msg_pack,
            )
...
impl MqttInterfaceOutput {
    ...
    pub fn answer(&mut self, input: String, lang: &LanguageIdentifier, to: String) -> Result<()> {
        self.client
            .try_send((SendData::String((input, lang.to_owned())), to))
            .unwrap();
        Ok(())
    }
